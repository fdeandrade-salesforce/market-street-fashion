/**
 * Organize product images from resources/50 products-01/ to public/images/products/{slug}/
 */
...
const manifestPath = path.join(__dirname, 'midjourney_manifest.csv')
const manifestContent = fs.readFileSync(manifestPath, 'utf-8')

const records = parseCSV(manifestContent)

const sourceDir = path.join(__dirname, 'resources', '50 products-01')
...
records.forEach((record: any) => {
  ...
  // Find matching source files (handle duplicates with _1, _2 suffixes)
  const basePattern = `${slug}-${imageNumber}`
  const matchingFiles = sourceFiles.filter(f => {
    const baseName = f.replace(/\.png$/, '')
    return baseName === basePattern || baseName.startsWith(`${basePattern}_`)
  })
  
  if (matchingFiles.length > 0) {
    // Use the first match (or the one without suffix if available)
    const exactMatch = matchingFiles.find(f => f === `${basePattern}.png`)
